<?php
	...
	if(!empty($_POST) && isset($_POST['recapchaPay'])){
	    ...
	    if($recaptcha->score >= 0.5 && $recaptcha->action=='pay') {
			...
			$d->reset();
			$d->query("select ten_$lang as ten,phiship,id from #_place_city where id='".$_POST['tinhthanh']."'");
			...
			$code_order = "DH".strtoupper(ChuoiNgauNhien(6));
			...
			if($mail->Send()){
				...
				transfer("Đặt hàng thành công ! ", 'san-pham');
			}else{
