<?php  if(!defined('_source')) die("Error");
		...
		if($id!=''){
			$d->reset();
			$d->query("select id,id_list,id_cat,ten_$lang,mota_$lang,noidung_$lang,title,keywords,description,photo,ngaytao from #_baiviet where id='".$id."' and hienthi=1");
		    ...
		    $breadcum .= "<i class='fa fa-angle-right' aria-hidden='true'></i><a href='".$row_detail['tenkhongdau']."'>".$row_detail['ten_'.$lang]."</a>";
			...
			$sql="select id,ten_$lang,mota_$lang,noidung_$lang,photo,title,keywords,description,ngaytao,tenkhongdau from";
			...
			$keywords_bar = $row_detail['keywords'];
			$description_bar = $row_detail['description'];
			$sql="select id,ten_$lang,mota_$lang,tenkhongdau,photo,ngaytao from";
			$startpoint = ($page * $per_page) - $per_page;
			$limit = ' limit '.$startpoint.','.$per_page;
