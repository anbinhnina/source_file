	...
	if(isset($_GET['id'])){
		...
		if(count($photo_lq)>0){
			for($i=0;$i<count($photo_lq);$i++){
				delete_file(_upload_album.$photo_lq[$i]['photo']);
				delete_file(_upload_album.$photo_lq[$i]['thumb']);
			}
		}
		...
		for ($i=0 ; $i<count($listid) ; $i++){
			...
			if(count($photo_lq)>0){
				for($j=0;$j<count($photo_lq);$j++){
					delete_file(_upload_album.$photo_lq[$j]['photo']);
					delete_file(_upload_album.$photo_lq[$j]['thumb']);
				}
			}	
			$d->query("delete from #_album_photo where id_album='".$id."'");
