...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_video,$file_name)){
			...
			$urlImage = "https://img.youtube.com/vi/".youtobi($_POST['links'])."/hqdefault.jpg";
			$arrCheck = getimagesize($urlImage);
			if($arrCheck){
				$file_name = fns_Rand_digit(0,9,15);
				$nameImg = $file_name.".jpg";
				if (!copy($urlImage, _upload_video.$nameImg)){
					if (!move_uploaded_file($urlImage,_upload_video.$nameImg)){
						return false;
					}
				}
				$data['photo'] = $nameImg;	
